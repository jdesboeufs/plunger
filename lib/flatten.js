...
const {remove, omit, uniq} = require('lodash')
...
function matchPatterns(tokens, options) {
  ...
  options.types.forEach(type => {
    const matchingTokens = rest.filter(token => type.extensions.some(ext =>
      token.fileTypes.some(type => type.ext === ext)
    ))
...
function flattenFiles(tokens, options) {
  ...
  tokens.forEach(token => {
    ...
    if (token.warning) {
      output.warnings.push(clean)
    }

    if (token.temporary) {
      output.temporary.push(token.temporary)
    }
    ...
    if (token.children) {
      ...
      output.warnings = output.warnings.concat(childOutput.warnings)
      output.temporary = uniq(output.temporary.concat(childOutput.temporary))
    }
  })
