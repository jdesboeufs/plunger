...
function getBaseUrls(location) {
  ...
  return {
    baseUrl: normalize(baseUrl, {stripWWW: false}),
    rootUrl: normalize(rootUrl, {stripWWW: false})
  }
}
    ...
    onopentag: (tag, attributes) => {
      ...
      if (isIndexOf && tag === 'a' && attributes.href) {
        ...
        if (parsedUrl.protocol) {
          ...
          fullUrl = `${rootUrl}/${attributes.href}`
        } else {
          fullUrl = `${baseUrl}/${attributes.href}`
        }

        fullUrl = normalize(fullUrl, {stripWWW: false})
